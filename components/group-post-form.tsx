...
import { useGroupPosts } from "@/hooks/use-groups"
...
export function GroupPostForm({ groupUri, groupName }: GroupPostFormProps) {
  const [content, setContent] = useState("")
  const [attachments, setAttachments] = useState<File[]>([])
  const [isPosting, setIsPosting] = useState(false)
  const { user } = useAuth()
  const { postToGroup } = useGroupPosts(groupUri)
  ...
  const handlePost = async () => {
    ...
    setIsPosting(true)
    try {
      await postToGroup(content, attachments)
      setContent("")
      setAttachments([])
      toast({
        title: "Post created",
        description: `Your post has been shared to ${groupName}.`,
      })
    } catch (error: any) {
